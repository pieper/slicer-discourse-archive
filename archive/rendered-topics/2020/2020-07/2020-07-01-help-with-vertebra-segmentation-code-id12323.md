# Help with Vertebra Segmentation Code

**Topic ID**: 12323
**Date**: 2020-07-01
**URL**: https://discourse.slicer.org/t/help-with-vertebra-segmentation-code/12323

---

## Post #1 by @vertebra (2020-07-01 15:07 UTC)

<aside class="onebox whitelistedgeneric">
  <header class="source">
      <img src="https://github.githubassets.com/favicons/favicon.svg" class="site-icon" width="16" height="16">
      <a href="https://github.com/kbehlmirusmed/VertebraSegmentation" target="_blank" rel="nofollow noopener">GitHub</a>
  </header>
  <article class="onebox-body">
    <img src="https://avatars2.githubusercontent.com/u/67699725?s=400&amp;v=4" class="thumbnail onebox-avatar" width="60" height="60">

<h3><a href="https://github.com/kbehlmirusmed/VertebraSegmentation" target="_blank" rel="nofollow noopener">kbehlmirusmed/VertebraSegmentation</a></h3>

<p>Contribute to kbehlmirusmed/VertebraSegmentation development by creating an account on GitHub.</p>


  </article>
  <div class="onebox-metadata">
    
    
  </div>
  <div style="clear: both"></div>
</aside>

<p>Please may someone help me sort through this code and give any edits!</p>

---

## Post #2 by @vertebrae (2020-07-01 19:13 UTC)

<p><a class="mention" href="/u/vertebra">@vertebra</a> and I would like to know if there are any problems or improvements that can be made. We are trying to use a pre-placed seed point and use the local threshold function to segment individual lumbar vertebrae, render it in 3D, and save it as an stl file.</p>

---
