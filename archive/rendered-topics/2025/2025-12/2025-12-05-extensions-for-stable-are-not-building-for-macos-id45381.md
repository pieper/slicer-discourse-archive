# Extensions for stable are not building for macos

**Topic ID**: 45381
**Date**: 2025-12-05
**URL**: https://discourse.slicer.org/t/extensions-for-stable-are-not-building-for-macos/45381

---

## Post #1 by @muratmaga (2025-12-05 21:57 UTC)

<p>No Mac extensions. Windows and Linux seems fine:</p>
<aside class="onebox allowlistedgeneric" data-onebox-src="https://slicer.cdash.org/index.php?project=SlicerStable">
  <header class="source">
      <img src="https://slicer.cdash.org/favicon.ico" class="site-icon" alt="" width="32" height="32">

      <a href="https://slicer.cdash.org/index.php?project=SlicerStable" target="_blank" rel="noopener nofollow ugc">slicer.cdash.org</a>
  </header>

  <article class="onebox-body">
    

<h3><a href="https://slicer.cdash.org/index.php?project=SlicerStable" target="_blank" rel="noopener nofollow ugc">SlicerStable</a></h3>

  <p>
            CDash is an open source, web-based software testing server. CDash aggregates, analyzes, and displays the
            results of software testing processes submitted from clients located around the world. CDash is a part of a
           ...</p>


  </article>

  <div class="onebox-metadata">
    
    
  </div>

  <div style="clear: both"></div>
</aside>

<p><a class="mention" href="/u/sam_horvath">@Sam_Horvath</a> <a class="mention" href="/u/ebrahim">@ebrahim</a></p>

---

## Post #2 by @ebrahim (2025-12-06 04:47 UTC)

<p>I see mac extension builds showing up now, maybe it was just delayed? (I don’t have access to computron atm.) There are a lot of stragglers showing up on Dec 6, and still rolling in as I write this</p>

---

## Post #3 by @muratmaga (2025-12-06 04:51 UTC)

<p>OK, looks like MacOS builds didn’t even start until couple hours ago. At this point I am not sure if this is soon to be yesterday’s build, or actually the build for tomorrow (Saturday). Looks like similar issue to this: <a href="https://discourse.slicer.org/t/slicer-build-factory-speed/45358" class="inline-onebox">Slicer build factory speed</a></p>

---
